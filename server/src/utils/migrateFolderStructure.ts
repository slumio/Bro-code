...
async function migrateFolderStructure() {
  try {
        ...
        if (file.parentId) {
          ...
          if (!(file.parentId instanceof mongoose.Types.ObjectId)) {
            ...
            const parentFile = room.files.find(pf => 
              pf._id.toString() === file.parentId || 
              pf.originalId === file.parentId
            );
            
            if (parentFile) {
              file.parentId = parentFile._id;
              file.parentOriginalId = parentFile.originalId || parentFile._id.toString();
              needsUpdate = true;
            } else {
    ...
    console.log(`Migration complete! Updated ${migratedRooms} out of ${rooms.length} rooms.`);
  } catch (error) {
